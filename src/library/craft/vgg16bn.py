...
class Vgg16Bn(torchNN.Module):
    ...
    def __init__(self, pretrained=True, freeze=True):
        ...
        if pretrained:
            ...
            state_dict = torch.hub.load_state_dict_from_url("http://download.pytorch.org/models/vgg16_bn-6c64b313.pth", progress=True)
            vgg.load_state_dict(state_dict)
            vgg_pretrained_features = vgg.features
        else:
            vgg_pretrained_features = torchvisionModel.vgg16_bn(weights=None).features
        ...
        if not pretrained:
            Vgg16Bn.init_weights(self.slice1.modules())
            Vgg16Bn.init_weights(self.slice2.modules())
            Vgg16Bn.init_weights(self.slice3.modules())
            Vgg16Bn.init_weights(self.slice4.modules())

        Vgg16Bn.init_weights(self.slice5.modules())
